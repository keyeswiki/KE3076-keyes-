...
按照接线图正确接好模块和各元器件，利用MicroUSB线连接到计算机上电，单击![Img](./media/52.png)来执行程序代码。代码开始执行，你会看到的现象是：外接LED灯不亮，同时Shell窗口中显示ADC值和"LED OFF!"；然后在水滴传感器模块上的感应区滴一两滴水，当水滴传感器检测水量大小输出的ADC值大于3000时，外接LED灯快速闪烁，同时Shell窗口中显示ADC值和"LED Blinking!"。
...
按照接线图正确接好模块和各元器件，利用MicroUSB线连接到计算机上电，单击![Img](./media/52.png)来执行程序代码。代码开始执行，你会看到的现象是：有源蜂鸣器不鸣叫，外接LED灯不亮，同时Shell窗口中显示ADC值和"Buzzer won't sound and led off!"；然后在水滴传感器模块上的感应区滴一两滴水，当水滴传感器检测水量大小输出的ADC值大于3000时，有源蜂鸣器发出警报，外接LED灯快速闪烁，同时Shell窗口中显示ADC值和“Buzzer sounds and led blinking!”。
